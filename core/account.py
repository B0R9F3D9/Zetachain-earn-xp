...
class Account:
    def __init__(self, account_id: int, key_mnemonic: str, proxy: str | None = None, withdraw_address: str | None = None) -> None:
        ...
        self.w3 = AsyncWeb3(AsyncWeb3.AsyncHTTPProvider(RPC), middlewares=[async_geth_poa_middleware], request_kwargs={'proxy': self.proxy})
        ...
        self.info = f"[№{self.account_id} - {self.addr}]"
    ...
    async def wait_until_tx_finished(self, hash: str, max_wait_time=60) -> None:
        ...
        while True:
            try:
                ...
                if status == 1:
                    logger.success(f"{self.info} Транзакция успешна! {self.explorer+hash}"); return
                elif status is None:
    ...
    @retry
    async def transfer_zeta(self, value: float, receiver_address: str | None = None) -> str:
